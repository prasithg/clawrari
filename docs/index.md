# Docs Index

- [Build Playbook](playbook.md)
- [Process](process.md)
- [Architecture](architecture.md)
- [Memory](components/memory.md)
- [Memory Promotion (Dreaming Pass)](memory-promotion.md)
- [The Harness](harness/README.md)
- [Self-Improvement](components/self-improvement.md)
- [Connectors](components/connectors.md)
- [Persona Patterns](components/persona.md)
- [Identity Channels](components/identity-channel.md)
- [Philosophy](philosophy.md)
- [Skills Catalog](../skills/README.md)
- [Crons](../crons/README.md)

Suggested reading order for a new install:

1. `playbook.md`
2. `architecture.md`
3. `process.md`
4. `components/memory.md`
5. `components/connectors.md`
6. `components/self-improvement.md`
7. `components/persona.md`
