...
For the layer that feeds the scorecard — tracing each run, capturing tokens and durable artifacts, and rendering a morning report a human reads in two minutes — see [Agent Observability](../observability.md). For how to compose these patterns into nested feedback loops where each one's signal feeds the next, see [Stacking Loops](../stacking-loops.md).
